...
- 精简版：节点选择、自动选择、WiFi Calling、广告、私有网络、国内、非中国和兜底
- 标准版：在精简版基础上增加 AI、YouTube、Google、Microsoft、Apple、GitHub、Telegram
- 完整版：启用除成人内容、Gemini 和 Google Scholar 之外的全部内置模块；这些模块仍可手动打开

导入节点时，节点名称中的独立国家/地区缩写会自动中文化，例如 `HK-01` 会显示为 `香港-01`、`JP 02` 会显示为 `日本 02`；生成 YAML 后，Mihomo 的节点选择列表直接使用这些中文名称。高级文本配置统一使用 `|` 分栏，界面内每一项均带有格式提示。例如：
