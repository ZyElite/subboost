...
- 逐节点分析 WiFi Calling 的 UDP 配置条件，并提示 UDP 500/4500 仍需通过实际代理出口测试
- 支持 SS、SSR、VMess、VLESS、Trojan、Hysteria2、TUIC、HTTP(S) 与 SOCKS5
- 自动处理重复节点名
- 精简版、标准版、完整版三套内置模板，可单独启停 36 个策略组
...
内置策略组的国家/地区筛选使用中文多选项，例如“香港、日本、美国”，模板 JSON 内仍使用兼容旧版本的稳定代号。高级文本配置统一使用 `|` 分栏，界面内每一项均带有格式提示。例如：
